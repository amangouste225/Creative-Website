...
if (!class_exists('WorkreapServiceMessage')) {
    ...
    class WorkreapServiceMessage extends Workreap_Email_helper{
		 ...
		 * @since 1.0.0
		 */
		public function send_service_message_freelancer($params = '') {
			extract($params);
			$subject_default = esc_html__('New Service Message Received', 'workreap_core');
			...
			if (function_exists('fw_get_db_settings_option')) {
				$subject 		= fw_get_db_settings_option('frl_service_msg_subject');
				$email_content  = fw_get_db_settings_option('frl_service_msg_content');
			}
            ...
            $email_content = str_replace("%message%", $service_msg, $email_content); 
        ...
        /**
		 * @Send employer service message
		 *
		 * @since 1.0.0
		 */
		public function send_service_message_employer($params = '') {
			extract($params);
			$subject_default = esc_html__('Service Message Received', 'workreap_core');
			...
			if (function_exists('fw_get_db_settings_option')) {
				$subject 		= fw_get_db_settings_option('emp_service_msg_subject');
				$email_content  = fw_get_db_settings_option('emp_service_msg_content');
			}
